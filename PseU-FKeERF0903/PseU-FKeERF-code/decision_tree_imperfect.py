...
class EDT(BaseEstimator, ClassifierMixin):
    ...
    def _build_tree(self, indices, root_node):
        # 首先按局部特征分裂
        A, threshold = self._best_gain(indices, feature_type='local')  # 处理局部特征
        ...
        # 然后按全局特征分裂
        A, threshold = self._best_gain(indices, feature_type='global')  # 处理全局特征
    ...
    def _best_gain(self, indices, feature_type=None):
        info_root = self._compute_info(indices)
        if info_root == 0:
            return None, None

        n_feat = self.X_trained.shape[1]
        gains = np.zeros(n_feat)
        thresholds = []

        # 1) 选择参与评估的特征索引（根据 feature_type 控制局部/全局）
        if feature_type == 'local':
            selected_features = list(range(0, n_feat // 2))  # 前半段视为局部
        elif feature_type == 'global':
            selected_features = list(range(n_feat // 2, n_feat))  # 后半段视为全局
        else:
            if self.rf_max_features == "sqrt":
                selected_features = list(np.random.choice(range(n_feat),
                                                          int(math.sqrt(n_feat)),
                                                          replace=False))
            else:
        ...
        for A in selected_features:
            ...
            if not flag_float:
                ...
                for v in np.unique(self.X_trained[indices, A]):
                    ...
                    part_info += (node.shape[0] / indices.shape[0]) * self._compute_info(node)

                    if node.shape[0] < self.min_samples_leaf:
                        part_info = info_root
                        break
            else:
                # 连续特征：调用原函数名 _find_treshoold（注意原始拼写）
                threshold, part_info = self._find_treshoold(indices, info_root, A)

            thresholds.append(threshold)

            gain = info_root - part_info
            gains[A] = gain if gain > 0 else 0.0

        # 3) 结果汇总
        if np.max(gains) == 0:
            return None, None

        best_feat = int(np.argmax(gains))
        # thresholds 列表与 selected_features 对齐，因此需要位置索引
        pos = selected_features.index(best_feat)
        return best_feat, thresholds[pos]
