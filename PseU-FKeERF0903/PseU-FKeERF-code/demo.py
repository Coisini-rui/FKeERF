...
#     local_features = data[:, :n_features // 2]  # 假设前一半是局部特征
#     global_features = data[:, n_features // 2:]  # 假设后一半是全局特征
#
#     # 合并局部和全局特征
#     processed_data = np.hstack((local_features, global_features))
#
...
def preprocess_data(data):
    ...
    # 使用 iloc 进行切片处理局部特征和全局特征
    local_features = data.iloc[:, :n_features // 2]  # 假设前一半是局部特征
    global_features = data.iloc[:, n_features // 2:]  # 假设后一半是全局特征

    # 合并局部和全局特征
    processed_data = pd.concat([local_features, global_features], axis=1)
...
if __name__ == '__main__':
    ...
    # data = pd.read_csv("M_cleaned.csv")
    # data = pd.read_csv("S_cleaned.csv")

    # 预处理数据：分离局部和全局特征
    data = preprocess_data(data)

    # 处理目标列，修改为 -1
    data.iloc[:, 0] = data.iloc[:, 0].replace(0, -1)

    # 转换为 NumPy 数组供后续操作
    features = data.iloc[:, 1:].values
    data = np.hstack((data.iloc[:, 0].values.reshape(-1, 1), features))

    acc, mcc, sn, sp = cross_validation(data)
